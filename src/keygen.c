...
int cecies_generate_secp256k1_keypair(cecies_SECP256K1_keypair* output, const uint8_t* additional_entropy, size_t additional_entropy_length)
{
    if (output == NULL)
    {
        cecies_fprintf(stderr, "\nCECIES: Key generation failed because the output argument was NULL!");
    ...
    uint8_t prvkeybuf[SECP256K1_PRIV_KEY_SIZE] = { 0x00 };
    uint8_t pubkeybuf[SECP256K1_PUB_KEY_SIZE] = { 0x00 };
    size_t prvkeybuflen = 0, pubkeybuflen = 0;
